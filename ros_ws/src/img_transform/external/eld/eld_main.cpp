...
int main(int argc, char *argv[]) {
    ...
    if(argc < 2 || argc > 4) {
        std::cerr << "Usage: " << argv[0] << " [input .g2o file]" << std::endl;
        exit(1);
    }
    ...
    if(argc >= 3) {
        ...
        if(alg == "CPL") {
            ...
            std::cerr << "The second argument must be either \"CPL\" or \"SE\""
                      << std::endl;
            exit(1);
        }
    }

    if(argc >= 4) { // this is filename for saving results
        write = true;
    }

    size_t num_poses;

    if(use_CPL) { // PGO with CPL-SLAM
        ...
        opts.reg_Cholesky_precon_max_condition_number = 2e06;
        ...
        if(write) {
            ...
            std::cout << "num of landmarks: " << measurements.num_landmarks << std::endl
                      << "num of poses: " << measurements.num_poses << std::endl;
