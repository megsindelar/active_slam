...
namespace CPL_SLAM
{
                            ...
                            const ComplexMatrix &Y0)
    {
        ...
        if (options.verbose)
        {
            ...
            {
                std::cout << " Using " << (problem.projection_factorization() == ProjectionFactorization::Cholesky ? "Cholesky" : "QR")
...
// Set number of threads
        ...
        if (options.verbose)
        {
            ...
            std::cout << "===== END CPL-SYNC =====" << std::endl
                      << std::endl;
        } // if (options.verbose)
    ...
    void CPL_SLAM(CPL_SLAMResult &CPL_SLAMResults, CPL_SLAMProblem &problem,
                const CPL_SLAMOpts &options,
                const ComplexMatrix &Y0)
    {
...
// Set number of threads
    ...
    void CPL_SLAM(CPL_SLAMResult &CPL_SLAMResults, 
                  ...
                  const CPL_SLAMOpts &options, const ComplexMatrix &Y0)
    {
        ...
        CPL_SLAM(CPL_SLAMResults, problem, options, Y0);
    }
